"""
The feature extraction process has been taken up from the EMBER project(https://github.com/endgameinc/ember).
Feature extraction based on general information such as header and sections has been removed since it cannot be trusted.
Modifications have been made here and there to make it work with my custom harness.

Kindly change the reshape parameters in 'predict.py', line 27 if you modify any features according to new feature dimensions.

Reference: https://github.com/endgameinc/ember/blob/master/ember/features.py
"""
...
class MinHashFeature(FeatureType):
    """
    Generate min_hash to compare object file with defined classes of malware.
    """
    ...
    def calc_hash(self, bytez):
        ...
        return minhashes
        """
        # Jaccard similarity comparison for a file array.
    ...
    def raw_features(self, bytez, lief_binary):
        """
        Returns a serial list with calculated minhashes.
        """

        raw_hash = {}
        raw_hash = self.calc_hash([bytez])
        for i in range(len(raw_hash)):
            return raw_hash[i].digest().tolist()
        
    def process_raw_features(self, raw_obj):
        """
        Returns a numpy array from serialized feature list.
        """
        
        serial_list = np.array(raw_obj, dtype=np.float32)
        return serial_list
...
class PEFeatureExtractor(object):
    ...
    features = [
        ByteHistogram(), ByteEntropyHistogram(), StringExtractor(), ImportsInfo(), ExportsInfo(), MinHashFeature(),
    ]
