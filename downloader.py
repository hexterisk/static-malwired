...
class MalShare:
    ...
    def __init__(self, typeClass, samples=100):
        """
    ...
    def download(self):
        ...
        # Start download from either a reference file or a download queue.
            ...
            for idx, entry in enumerate(self.download_queue):
                ...
                print(f"[~] Downloading {self.typeClass}({idx}/{len(self.download_queue)}): {entry}")
...
def BuildDatabase():
    ...
    for typeClass in config.Classes:
        objects[typeClass] = MalShare(typeClass) # Pass the number of samples to download for the particular class.
        try:
